...
migration_rates = np.logspace(start=-1, stop = 2, num = 3)
...
for num_islands in list_islands: 
  ...
  for migration_rate in migration_rates:
    ...
    afs_runtimes.append([f'{num_islands}i_ms_runtime_M{migration_rate}']  + runtime_ms) 
    afs_runtimes.append([f'{num_islands}i_qmd_runtime_M{migration_rate}'] + runtime_qmd)

afs_runtimes = afstools.transposed(afs_runtimes)
datetime = afstools.datetime_tag()
output_filename = os.path.join('csv-files', f'{datetime}_runtimes_sampling={sampling_type}_omega={omega}_theta={mutation_rate}.csv')
afstools.write_csv(afs_runtimes, output_filename, ',')

#######DRAWING GRAPH#########
filename = os.path.join('csv-files',f'*_runtimes_sampling={sampling_type}_omega={omega}_theta={mutation_rate}.csv')
...
for num_islands in list_islands:
  fig.add_trace(go.Scatter(x=list(data['samples_size']),
                               y=list(data[f'{num_islands}i_qmd_runtime_M{migration_rate}']),
                               mode='lines+markers',
...
fig.update_layout(
  title=f'Compare runtime of different numbers of islands: sampling_scheme = {sampling_type}',
